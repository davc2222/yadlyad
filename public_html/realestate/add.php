...
if (empty($_SESSION['user_id'])) {
    $redirect = urlencode('/realestate/add.php');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $totalFloors = $form['total_floors'] !== ''
        ? (int) $form['total_floors']
        : null;

    $squareMeters = $form['square_meters'] !== ''
        ? max(0, (int) $form['square_meters'])
        : null;

    $entranceDate = $form['entrance_date'] !== ''
        ? $form['entrance_date']
        : null;
    ...
    if ($error === '') {
        ...
        try {
            ...
            header(
                'Location: /realestate/my_ads.php?published=1&id=' . $adId
            );
            exit;
            ...
            $error = 'אירעה שגיאה בשמירת המודעה: ' . $e->getMessage();
        }
    }
}
...
<?php endif; ?>
    ...
    <div class="realestate-form-header">
        <h1>פרסום מודעת נדל״ן</h1>
        <p>מלא את פרטי הנכס והמודעה תועבר לאישור.</p>
    </div>
...
<div class="realestate-field realestate-field-full">
    ...
    <div id="imagePreviewGrid" class="realestate-image-preview-grid"></div>
        ...
        <div class="realestate-form-actions">
            ...
            <a href="/realestate/index.php" class="realestate-cancel-button">
                ביטול
            </a>
        </div>
