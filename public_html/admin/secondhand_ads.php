...
if ($id > 0 && in_array($action, ['approve', 'delete'], true)) {
    ...
    ");
        ...
        if ($ad = $stmt->fetch(PDO::FETCH_ASSOC)) {
...
function statusUrl($status, $q = '')
{
    ...
    return '/admin/secondhand_ads.php' . ($params ? '?' . http_build_query($params) : '');
}
...
function actionUrl($action, $id, $status, $q = '')
{
    ...
    return '/admin/secondhand_ads.php?' . http_build_query($params);
}
?>
...
<section class="vehicle-list-page vehicle-admin-page">
    ...
    <div class="vehicle-list-header">
        <div>
            <h1>ניהול מודעות יד שנייה</h1>
            <p>סה״כ
                <?= (int) ($counts['total'] ?? 0) ?> מודעות פעילות במערכת
            </p>
        </div>
    </div>
    ...
    <div class="vehicle-admin-tabs">
        ...
        <a class="vehicle-admin-tab pending<?= activeTab($status, 'pending') ?>"
        ...
        <a class="vehicle-admin-tab approved<?= activeTab($status, 'active') ?>"
        ...
        <a class="vehicle-admin-tab rejected<?= activeTab($status, 'inactive') ?>"
    ...
    <form class="vehicle-admin-search" method="get" action="/admin/secondhand_ads.php">
    ...
    <?php else: ?>
        ...
        <div class="vehicle-grid">
            <?php foreach ($ads as $ad): ?>
                <?php
                $adTitle = trim($ad['title'] ?? '');

                if ($adTitle === '') {
                    $adTitle = 'מודעת יד שנייה';
                }
                ...
                <div class="vehicle-card">
                    ...
                    <div class="vehicle-card-image">
                        ...
                        <?php if (!empty($ad['item_condition'])): ?>
                            <span class="year-badge">
                                <?= h(conditionText($ad['item_condition'])) ?>
                            </span>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="vehicle-card-body">
                        ...
                        <div class="vehicle-meta">
                            ...
                            <?php if (!empty($ad['subcategory_name'])): ?>
                                <span>
                                    <?= h($ad['subcategory_name']) ?>
                                </span>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="vehicle-admin-owner">
                            ...
                            <?= h($created) ?>
                            | צפיות:
                            <?= number_format((int) ($ad['views'] ?? 0)) ?>
                        </div>

                        <div class="vehicle-admin-actions">

                            <a class="admin-btn-view" href="/secondhand/view.php?id=<?= (int) $ad['id'] ?>" target="_blank">
                                👁 צפה
                            </a>
